...
Checkinitem::Checkinitem(QWidget *parent,QString n) :
    QMainWindow(parent),
    ui(new Ui::Checkinitem)
{
    ui->setupUi(this);
    name =n;
    ui->label_4->hide();
...
void Checkinitem::on_pushButton_clicked()
{
    qDebug()<<"1";
    ...
    if(ui->comboBox_2->currentText()=="clerk")
    {
        qDebug()<<"1";
        ...
        if(db.open())
        {
            qDebug()<<"3";
        QSqlQuery query;
        query.prepare("select * from clerks where name = ?");
